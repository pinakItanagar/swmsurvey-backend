...
// SELECT count(p_record_id) FROM `property_master_update` WHERE `is_replicate` = '0' AND `property_pic` != "" 
...
$myfile = fopen("/var/www/html/psc/download/cdb_datatransfer_new.log", "w") or die("Unable to open file!");
    ...
    $sqlQR = "SELECT  property_master_update.*, master_building_type.building_type_name, master_circle.circle_name, master_surveyor.first_name, master_surveyor.last_name, "
            ...
            . "property_master_update.building_type = master_building_type.use_code  AND is_replicate = '0' AND  "
            . "master_circle.circle_id = property_master_update.circle_id AND property_master_update.updated_by_user = master_surveyor.userId  AND property_pic != '' ORDER BY p_record_id ASC LIMIT 100 ";
