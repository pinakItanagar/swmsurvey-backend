...
class Dashboard extends CI_Controller {
    ...
    public function __construct() {
        ...
        //$this->load->model('Master_ivr', 'tblIvr');
        //$this->load->model('Master_patient', 'tblpatient');
    }
    ...
    public function index_old() {
        ...
        $allWard =  $this->tblDashboard->allSurveyWardMap();
      
        $activeWards = array();
        foreach ($allWard as $rx) {
          
            $ward_boundary_file = base_url("upload/wardboundary/".$rx->ward_boundary_file); 
            
            
            
            $temp = array(
                "ward_no" => $rx->ward_no,
                "ward_color" => $rx->ward_color,
                "vendor_name" => $rx->vendor_name,
                "total_footprint" => $rx->total_footprints ,
                "ward_boundary_file" => file_get_contents($ward_boundary_file)
            );
            
            array_push($activeWards,$temp);
            
        }
        $data['wardmap'] = json_encode($allWard);
        $data['activeWards'] = $activeWards;
    ...
    public function index() {
        ...
        $file_week_data = "download/weekly_data_".$today.".json";
        $weekdata_file = base_url($file_week_data);
        $weekdata = file_get_contents($weekdata_file);
        //$weekdata = $this->tblDashboard->getWeekData();
    ...
    public function getWardMapDetails($ward_no ) {
        $ward = $this->tblDashboard->allSurveyWardMapByWardno($ward_no);
        $boundary_file =   "upload/wardboundary/".trim($ward->ward_boundary_file)  ;   
        $ward_boundary_details = base_url($boundary_file);
        $ward_boundary = file_get_contents($ward_boundary_details);
        $ward_color = trim($ward->ward_color);
       
        $response['ward_no'] = $ward_no;
        $response['ward_boundary'] = json_decode($ward_boundary);
        $response['ward_color'] = $ward_color;
        $response['vendor_name'] = $ward->vendor_name;
        $response['total_footprint'] = $ward->total_footprints;
        $response['total_survey'] =  $this->tblDashboard->totalSurveyDone($ward_no);
        $response['total_survey_today'] =  $this->tblDashboard->totalSurveyDoneToday($ward_no);
    ...
    public function viewnewproperty($pid, $img_storage_type) {
      
      $pagename = "property/viewNewProperty";
      $title = "View Property Details";
      $property = $this->tblDashboard->allNewPropertyData($pid, $img_storage_type);
     ...
     public function viewExistingproperty($pid, $img_storage_type) {
