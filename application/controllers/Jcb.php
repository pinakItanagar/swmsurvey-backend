...
class Jcb extends CI_Controller {
    ...
    public function deleteAllJcbData() {
        ...
        $data = $this->couchdb->getCouchDbAllDocs($payload,"_design/survey/_view/getAllJCBData?&ascending=true&key="); 
        $sensor_data = $data->rows;
        $total = count($sensor_data);
        for($i=0; $i<$total; $i++) {
            $rev = $sensor_data[$i]->value->_rev ;
            $doc_id = $sensor_data[$i]->value->doc_id ;
            $res =   $this->couchdb->deleteCouchDbDoc($rev,$doc_id, 'application/json' );
            echo $res."<br>";
