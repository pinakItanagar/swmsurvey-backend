...
<script type="text/javascript">
   ...
   function polystyle(wardColor) {
    ...
    var osm = L.tileLayer('http://{s}.google.com/vt/lyrs=p&x={x}&y={y}&z={z}',{ maxZoom: 20, subdomains:['mt0','mt1','mt2','mt3']});
       ...
       <? for($i=0; $i<count($activeWards); $i++) { ?>
          <? if($activeWards[$i]['ward_boundary_file'] != "") { ?>
           ward_boundary_geojson = <?=$activeWards[$i]['ward_boundary_file']?> ;
           ward_color = '<?=$activeWards[$i]['ward_color']?>' ;
           vendor_name = '<?=$activeWards[$i]['vendor_name']?>' ;
           total_footprint = '<?=$activeWards[$i]['total_footprint']?>' ;
           ward_no = '<?=$activeWards[$i]['ward_no']?>' ;
           
           var wno = ward_boundary_geojson;
           ...
           url = "<?=base_url('dashboard/overallwardinfo/')?>" + ward_no;
           ...
           marker.bindTooltip(ward_name, {permanent: true,  offset: [0, 0] }).bindPopup("Vendor Name : " + vendor_name + "<br> Available Building Footprints : " + total_footprint + "<br> As on date QR Code installed : " + total_qrcode_installed + "<br> QR Code installed Today : " + total_qrcode_installed_today + '<br><a href="'+ url + '">View Survey Details</a>'  );
           marker.addTo(map);*/
    
           var marker = new L.marker([lat, lng]); //opacity may be set to zero
           marker.bindTooltip(ward_name, {permanent: true,  offset: [0, 0] }).bindPopup("Vendor Name : " + vendor_name + "<br> Available Building Footprints : " + total_footprint + '<br><a href="'+ url + '">View Survey Details</a>'  );
           marker.addTo(map);
         <? } ?>  
...
<script type="text/javascript">
   ...
   /*
    ...
    var osm = L.tileLayer('http://{s}.google.com/vt/lyrs=p&x={x}&y={y}&z={z}',{ maxZoom: 20, subdomains:['mt0','mt1','mt2','mt3']});
        ...
        var coordinates = "";
        var vendor_name = "";
        var total_qrcode_installed = "";
        var total_qrcode_installed_today = "";
        var total_footprint = "";
        var url = ""; 
        var ward_no = ""; 
        ...
        <? for($i=0; $i<count($activeWards); $i++) { ?>
          <? if($activeWards[$i]['ward_boundary_file'] != "") { ?>
           ...
           ward_color = '<?=$activeWards[$i]['ward_color']?>' ;
           vendor_name = '<?=$activeWards[$i]['vendor_name']?>' ;
           total_qrcode_installed = '<?=$activeWards[$i]['total_qrcode_installed']?>' ;
           total_qrcode_installed_today = '<?=$activeWards[$i]['total_qrcode_installed_today']?>' ;
           total_footprint = '<?=$activeWards[$i]['total_footprint']?>' ;
           ward_no = '<?=$activeWards[$i]['ward_no']?>' ;
